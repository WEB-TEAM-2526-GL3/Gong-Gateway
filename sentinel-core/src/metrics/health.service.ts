...
export class HealthService {
  ...
  private subscribeToMetrics(): void {
    this.eventEmitter.on('metrics.updated', (event: {
      clientId: string;
      providerId: string;
      ...
      if (hasErrors) {
        ...
        if (newCount >= 10 && this.healthState.get(event.providerId) !== false) {
  ...
  private hasErrorStatus(statusCodes: Record<string, number>): boolean {
    return Object.keys(statusCodes).some(code => code.startsWith('5') || code === '429');
  }
