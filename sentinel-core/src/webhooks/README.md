...
`WebhooksModule` is Sentinel's outbound notification module. It lets admins
...
| `guards/webhook-admin.guard.ts` | Protects admin routes with `X-Sentinel-Admin-Key`. |
...
3. Adds headers:
   ...
   - `X-Sentinel-Signature: sha256=<hmac>` when a secret exists
