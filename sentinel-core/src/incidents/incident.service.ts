...
export class IncidentService {
  ...
  private subscribeToEvents(): void {
    ...
    this.eventEmitter.on('health.changed', async (event: {
      providerId: string;
      healthy: boolean;
    }) => {
    ...
    this.eventEmitter.on('limit.exceeded', async (event: {
      clientId: string;
      providerId: string;
      limitId: string;
      ...
      if (!primaryLink || primaryLink.providerId !== event.providerId) {
        ...
        const secondary = (await this.linkRepo.findByClient(event.clientId))
          .find(l => l.providerId === event.providerId && l.kind === 'secondary-active');
        if (secondary) {
          const incident = await this.incidentRepo.create({
          ...
          await this.linkRepo.save({ ...secondary, kind: 'secondary-inactive', incidentId: incident.id });
          this.eventEmitter.emit('incident.created', incident);
        }
        return;
      }
