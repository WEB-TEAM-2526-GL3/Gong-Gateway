# Gong Gateway — Architecture Overview (Front to Back)
...
2. **Admin registers a provider (e.g. OpenAI) in Kong** → Gong creates a Kong service + route + injects the API key as a header plugin
3. **Admin creates a consumer (internal team)** → Gong creates a Kong consumer + key-auth credential, returns the API key
...
6. **Prometheus scrapes Kong /metrics every 15s** → Gong polls Prometheus every 15s, caches GatewayMetrics, tracks health
...
| `POST/GET /webhooks`                              | `X-Gong-Admin-Key` | CRUD webhook configs                                                     |
...
Prometheus scrapes every 15s (configured in prometheus.yml)
       │
       ▼
Gong PrometheusService queries Prometheus HTTP API every 15s
...
Admin → Gong :3000 → Kong Admin API :8001 (configure services/routes/consumers)
Prometheus → scrapes Kong :8001/metrics ← Gong polls Prometheus :9090
